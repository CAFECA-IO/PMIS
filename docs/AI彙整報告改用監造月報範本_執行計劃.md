...
> 日期：2026-08-04
> 前置：`監造月報範本.md`（五層式版面，已定案）、`AI彙整報告生成流程重新設計.md`（白名單數據集流程，Phase A–C 已完成）
...
| 項目 | 決定 |
| --- | --- |
| 欄位缺口 | **分階段**：本次僅補「契約工期」一個欄位（工程概要改條列後已無缺口）；~~「逐工項本月完成」先顯示 `—` 並註明需月結快照，下一階段再做~~ → **已於 2026-08-06 補齊**，改由日報數量表加總取得（決策 A），快照方案取消 |
...
**C. ~~需月結快照（本次不做，顯示 `—`）~~ → 已於 2026-08-06 以日報數量表實作**

（以下為當時判斷，保留說明脈絡）「逐工項本月完成百分比／金額」需要上期存底才能算差額，但 `WorkItem.completedQty` 是單一累計值、會被覆寫，schema 無任何快照或歷史表。本次於表格該欄顯示 `—`，並在報告加註「本期完成需月結快照，功能開發中」。同理，`custom-progress` 圖的「本期增量」欄留空（該圖已支援只給累計值）。

> **【2026-08-06 已解決，且未採快照】**
> `SupervisionReportItem`（監造日報數量表，E1）上線後，本期完成 = **期間內該工項 `dailyQty` 之和**，
> 累計完成 = 期初 + 全期間加總。日報本身就是流水帳，毋須另建 `WorkItemPeriodSnapshot`。
> 3.3 的本期兩欄與 `custom-progress` 的第三欄（本期增量）皆已有真實數值。
> 僅計入已提送／已核備的日報（決策 G）；本期無紀錄時仍顯示 `—`，
> 因為「無紀錄」與「本期為 0」意義不同。詳見 `日報月報計劃整合檢視.md`。
...
> **【2026-08-06 更新】** 3.3 亦已補齊，改由日報數量表加總取得，快照方案取消。
...
- ~~**「本期完成」暫缺**：工項明細表該欄顯示 `—`。~~ → **已解決（2026-08-06）**：改由日報數量表加總取得。惟若該期間無已提送／已核備的日報數量紀錄，該欄仍為 `—`（此時報表會加註原因），因為「無紀錄」不等於「本期為 0」。
