...
test("除基本資料外的段落都依賴契約", () => {
  ...
  assert.equal(requiresDocument("packages"), true);
  assert.equal(requiresDocument("obligations"), true);
  assert.equal(requiresDocument("owners"), true);
  assert.equal(requiresDocument("workItems"), true);
  assert.equal(
...
test("明確指定段落時一律照辦，優先於其他規則", () => {
  ...
  // 即使附了檔案，指定單段也不擴大範圍
  assert.deepEqual(
    resolveScope({ only: ["workItems"], hasAttachment: true, hasPriorRun: true }),
    ["workItems"],
  );
});
...
test("指定的段落順序一律正規化，不受呼叫端傳入順序影響", () => {
  assert.deepEqual(
    resolveScope({
      only: ["workItems", "profile", "obligations"],
      hasAttachment: false,
      hasPriorRun: true,
    }),
    ["profile", "obligations", "workItems"],
  );
});
...
test("完全沒有契約時，各段皆略過並說明補救方式", () => {
  for (const step of ["scope", "obligations", "owners", "packages", "workItems"] as const) {
...
test("推導型段落需要履約標的作為上游輸入", () => {
  ...
  assert.equal(requiresScope("packages"), true);
});

test("照抄型與不相關的段落不需要履約標的", () => {
  assert.equal(requiresScope("scope"), false, "階段一自己就是產生者");
  assert.equal(requiresScope("profile"), false);
  assert.equal(
    requiresScope("workItems"),
    false,
    "階段三的上游是工程項目，不是履約標的",
  );
});
