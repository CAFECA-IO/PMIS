...
/**
 * Info: (20260806 - Julian) 月報組裝的行為測試。
 *
...
/** Info: (20260806 - Julian) 預定 2026-01-01 ~ 2026-12-31、契約量 100 的工項。 */
const wi = (
  id: string,
...
// Info: (20260806 - Julian) ── 決策 G：草稿不計入，且必須被揭露 ────────────────────────

test("草稿日報不進施工天數，也不進逐日明細", () => {
  /*
    Info: (20260806 - Julian) 先前只在數量側套決策 G：4.1 的施工天數與 4.2 的逐日明細照樣含草稿，
...
test("累計欄位只用傳入的「截至期末」加總，不受本期加總影響", () => {
  /*
    Info: (20260806 - Julian) 補產舊月報時，累計必須是期末當下的值。呼叫端負責把上限傳對
...
// Info: (20260806 - Julian) ── 本期「0」與「無資料」是兩件事 ───────────────────────────
...
// Info: (20260806 - Julian) ── 預定與完成必須算在同一批工項上 ──────────────────────────

test("無預定起訖日的工項不進整體進度比對，且排除數要揭露", () => {
  /*
    Info: (20260806 - Julian) 先前完成側納入、預定側排除：1 個排程工項做完 100%、
  ...
  */
  ...
  /*
    Info: (20260806 - Julian) 只有 done 進入比對，所以完成是 100 而不是被三個 0 稀釋成的 25。
  ...
  */
  ...
  // Info: (20260806 - Julian) 未納入比對不等於被忽略：3.3 仍列出全部四項
  assert.equal(r.template.workItems.length, 4);
});

test("完全沒有具預定起訖日的工項時，累計預定與完成皆為 null 而非 0", () => {
  /*
    Info: (20260806 - Julian) 先前以 0 代入，於是沒填預定日的專案會印出「累計預定 0.00%」
...
test("同一批工項時，累計預定與完成的母體一致", () => {
  // Info: (20260806 - Julian) 全部工項都有預定日 → 排除數為 0，兩側算在同一批上
  const r = build({
...
// Info: (20260806 - Julian) ── 缺契約數量：百分比為 null 但金額仍可能有值 ──────────────
...
// Info: (20260806 - Julian) ── 專案基本資料原樣傳遞 ────────────────────────────────────

test("Decimal 型別的契約金額於邊界轉為 number", () => {
  const r = build({
    project: {
      ...build().template.project,
      // Info: (20260806 - Julian) Prisma Decimal 在邊界會以字串形式出現
