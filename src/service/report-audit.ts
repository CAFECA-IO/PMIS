...
import type { AuditRowData } from "@/repository/supervisionReport.repository";
import type { ReportStatus } from "@/generated/prisma/enums";

/**
 * Info: (20260806 - Julian) 日報變更軌跡的差異描述（純函式，無 I/O，便於單元測試）。
 *
 * 目的是讓「改了什麼」看得懂：只記「被更新過」等於沒記，
 * 對帳時仍無從說明月報數字為何改變。
 */

/** Info: (20260806 - Julian) 參與比對的日報欄位；值一律先轉為可比較的字串或 null。 */
export type ComparableFields = Record<string, string | null>;

/** Info: (20260806 - Julian) 欄位的中文標題，供描述使用；未列者以欄位名呈現。 */
...
/** Info: (20260806 - Julian) 過長的值截斷，避免單筆軌跡塞入整段敘述。 */
const clip = (v: string, max = 60): string =>
  v.length <= max ? v : `${v.slice(0, max)}…`;

/**
 * Info: (20260806 - Julian) 比對欄位並產生「欄位：舊 → 新」的描述；無異動時回 null。
 *
...
export function describeFieldChanges(
  ...
  return {
    ...
    // Info: (20260806 - Julian) 保存整份變更前欄位，而非只存有變動者：重建不該還要跨筆推敲哪些沒變
    before: JSON.stringify(before),
  };
}

/**
 * Info: (20260806 - Julian) 數量表一列（比對與留存用的最小欄位）。
 *
...
/**
 * Info: (20260806 - Julian) 一筆軌跡的內容：摘要給人看，JSON 供回溯重建。
 *
 * 摘要會截斷（軌跡列表若塞滿整段敘述就沒人會讀），**因此 JSON 不可省略** ——
 * 截斷過的摘要不是紀錄，是提示。
 */
export type AuditDetail = {
  /** Info: (20260806 - Julian) 供人閱讀的摘要；過長的值已截斷。 */
  summary: string;
  /** Info: (20260806 - Julian) 變更前（或建立時）的完整內容 JSON，未截斷。 */
  before: string;
};

/** Info: (20260806 - Julian) @deprecated 沿用舊名，語意同 AuditDetail。 */
export type QtyChange = AuditDetail;

/** Info: (20260806 - Julian) 台帳工項以 workItemId 為身分；契約外項目沒有身分，只能以名稱成組比對。 */
const isLedgerRow = (r: QtySnapshotRow): r is QtySnapshotRow & {
  workItemId: string;
} => Boolean(r.workItemId);

/** Info: (20260806 - Julian) 一列的可比較內容（不含身分）。 */
const rowSig = (r: QtySnapshotRow) =>
  `${r.dailyQty}|${r.unit ?? ""}|${r.note ?? ""}`;

/** Info: (20260806 - Julian) 契約外同名列的整組簽章；排序後比較，與填寫順序無關。 */
const groupSig = (rows: QtySnapshotRow[]) =>
  rows.map(rowSig).sort().join("\u0000");

/** Info: (20260806 - Julian) 契約外同名列的顯示：`3、7m`。 */
...
/**
 * Info: (20260806 - Julian) 比對數量表並產生變更描述；無異動時回 null。
 *
...
export function describeQtyChanges(
  ...
  /*
    Info: (20260806 - Julian) ── 台帳工項：以 workItemId 成組比對 ────────────────────────
  ...
  */
  ...
  for (const [id, a] of afterById) {
    ...
    if (a.length === 1 && b.length === 1) {
      // Info: (20260806 - Julian) 數量、單位、備註各自比對：只看數量會讓改單位／改備註完全不留痕
      const diffs: string[] = [];
  ...
  // Info: (20260806 - Julian) ── 契約外項目：同名成組比對 ──
...
/**
 * Info: (20260806 - Julian) 建立一份日報時的初始內容描述。
 *
...
export function describeCreation(
  ...
  return {
    ...
    // Info: (20260806 - Julian) 同 describeFieldChanges：摘要會截斷，故一併留下未截斷的初始內容
    before: JSON.stringify({ fields, items }),
  };
}

/**
 * Info: (20260806 - Julian) 刪除一份日報時的完整描述。
 *
...
export function describeDeletion(input: {
  ...
  // Info: (20260806 - Julian) 免計工期一律列示，即使為「否」—— 沒有宣告本身也是需要留存的事實
...
/** Info: (20260806 - Julian) 依是否有欄位／數量異動決定要寫哪些軌跡動作。 */
...
/**
 * Info: (20260806 - Julian) 把一次儲存轉換成要寫入的軌跡列（純函式）。
 *
 * 這一段先前住在 `supervisionReport.service`，只能以原始碼字串比對守著
 * ——例如比對整個檔案是否含 `detail: fieldChanges!.summary`。
 * 那種斷言擋不住真正的失誤：把 UPDATE 分支換成 `snapshot: qtyChanges!.before`
 * （對的欄位配錯的來源）照樣全綠，因為兩個字面值都還在檔案裡的某處。
 * 移到純函式後即可用真實的前後值驗證每一種動作各自帶了什麼。
 */
export function buildAuditRows(input: {
  reportId: string;
  projectId: string;
  /** Info: (20260806 - Julian) 該日報的報表日期；使軌跡在日報刪除後仍看得出是哪一天。 */
  reportDate: Date;
  ...
  return actions.map((action) => {
      ...
      if (action === "ITEMS") {
        return {
          ...base,
          action,
          // Info: (20260806 - Julian) 保存變更「前」的明細：變更後的值讀現況即得，變更前的一旦覆寫即永久消失
      ...
      if (action === "CREATE") {
        /*
          Info: (20260806 - Julian) CREATE 先前只寫下 action 而 detail 為 null（新建無「變更前」可比對），
      ...
      // Info: (20260806 - Julian) UPDATE：摘要中的值會截斷，故一併保存未截斷的變更前欄位
      return {
        ...base,
        action,
