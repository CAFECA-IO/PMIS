...
/**
 * Info: (20260806 - Julian) 期間鍵是彙整報表留存的身分：`upsertDraft` 以它決定覆寫哪一份草稿，
...
/** Info: (20260806 - Julian) 在指定時區下執行；還原原本的 TZ，避免測試之間互相影響。 */
...
/** Info: (20260806 - Julian) 涵蓋正、零、負三種 UTC 偏移；負偏移是先前出錯的那一類。 */
const ZONES = ["Asia/Taipei", "UTC", "America/New_York"];

// Info: (20260806 - Julian) ── parseRefDate：純日期一律解析為本地日曆日 ──────────────────
...
test("parseRefDate 拒絕不存在的日期而非靜默進位", () => {
  // Info: (20260806 - Julian) new Date(2026, 12, 1) 會變成 2027 年 1 月；那會讓使用者拿到別年的報表
...
// Info: (20260806 - Julian) ── periodKeyFor：同一個日曆日在任何時區都得到同一個鍵 ──────────
...
test("週鍵取該週的星期一", () => {
  // Info: (20260806 - Julian) 2026-08-05 是星期三
