...
/**
 * Info: (20260806 - Julian) Local file storage service. All file reads/writes go through here so the
 * rest of the app never touches the filesystem directly. Swap the
 * implementation (e.g. S3) without changing callers.
 */
/*
 * Info: (20260806 - Julian) turbopackIgnore：process.cwd() 會讓 Turbopack 把整個專案納入輸出追蹤
...
/**
 * Info: (20260806 - Julian) 檔案選擇器的 accept 字串。
 *
...
/**
 * Info: (20260806 - Julian) 這個 MIME 的檔案是否可能被 saveFile 接受。
 *
...
/**
 * Info: (20260806 - Julian) 直接以位元組寫入儲存區，供 JSON／base64 來源使用（費思對話的附件）。
 *
...
/**
 * Info: (20260806 - Julian) 服務手動上傳元件。HEIC／HEIF 會先轉為 JPEG 再存檔。
 *
...
export async function saveFile(file: File): Promise<SavedFile | null> {
  ...
  // Info: (20260806 - Julian) 轉檔後才查白名單：HEIC 於此時已成 image/jpeg
...
export async function read(storedName: string): Promise<Buffer | null> {
  const safe = path.basename(storedName); // Info: (20260806 - Julian) guard against path traversal
  try {
