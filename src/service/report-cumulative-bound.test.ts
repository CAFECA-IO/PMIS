...
/**
 * Info: (20260806 - Julian) 報表數字不得靜默改寫使用者沒看到的內容。
 *
...
// Info: (20260806 - Julian) ── 累計的時間上限 ──────────────────────────────────────────

test("月報不得使用無上限的日報累計", () => {
  const source = read("src/service/report.service.ts");
  /*
    Info: (20260806 - Julian) 只比對呼叫形式，不比對註解 —— 本檔的說明文字本來就會提到這個函式名。
...
test("月報的有效進度以期末為界取得，且與其他查詢同基準", () => {
  /*
    Info: (20260806 - Julian) 這條守門原本寫的是 `getWorkItemDetails(projectId, end)` —— 也就是把缺陷
...
// Info: (20260806 - Julian) ── 留存的必須是畫面上那一份 ────────────────────────────────
...
test("留存不得另跑一次產製", () => {
  /*
    Info: (20260806 - Julian) 產製與留存必須在同一次呼叫內完成：報表數字是即時推導、期間評述由 LLM 現寫，
...
test("留存的內容讀得回來", () => {
  // Info: (20260806 - Julian) 只列 metadata 而讀不到全文，等於存進去再也打不開
...
// Info: (20260806 - Julian) ── 產製是明示動作，不得自動發生 ──────────────────────────

/** Info: (20260806 - Julian) 取出 `from` 之後到下一個 `export ` 之間的內容（近似函式本體）。 */
...
/** Info: (20260806 - Julian) 去掉註解，讓原始碼的比對不被註解干擾。 */
function stripComments(source: string): string {
  return source.replace(/\/\*[\s\S]*?\*\//g, "").replace(/^\s*\/\/.*$/gm, "");
}

/** Info: (20260806 - Julian) 取出所有 useEffect 的內容（以括號配對掃描，避免依賴縮排）。 */
...
test("開啟頁面或改參數不得自動產製報表", () => {
  /*
    Info: (20260806 - Julian) 產一份報表要呼叫 LLM（付費）並寫一列留存。先前掛載與每次改參數都自動跑，
...
test("基準日在伺服器端守門，不合理者拒絕而非退回今天", () => {
  /*
    Info: (20260806 - Julian) `parseRefDate` 已移到零相依的 `period-key.ts`（與期間鍵同住 ——
  ...
  */
  ...
  // Info: (20260806 - Julian) 用戶端防抖擋不住直接打 API 的請求，守門必須在伺服器端
  assert.match(read("src/app/api/report/route.ts"), /parseRefDate/);
});

test("定稿不與草稿共用同一個截斷", () => {
  /*
    Info: (20260806 - Julian) 留存清單是唯一能開啟定稿的 UI。若定稿與草稿共用一個 take，
...
// Info: (20260806 - Julian) ── 定稿只能凍結使用者讀過的那一版 ──────────────────────────

test("確認定稿必須帶版本，且伺服器端比對", () => {
  /*
    Info: (20260806 - Julian) 草稿是同一列原地覆寫，所以 id 不足以指明「哪一版」：
...
test("展開的留存內容在清單重載時重新驗證版本", () => {
  /*
    Info: (20260806 - Julian) 先前 opened 只在展開時設定一次：別處重新生成後，列上的「產生於」
...
// Info: (20260806 - Julian) ── 新建不得覆蓋既有日報 ────────────────────────────────────

test("新建日報撞到既有日期時必須拒絕，不得改為更新", () => {
  /*
    Info: (20260806 - Julian) 新建表單的文字欄位一律從空白開始（它不顯示既有內容），送出時
...
test("新建表單在選定日期當下就提示衝突", () => {
  // Info: (20260806 - Julian) 伺服器端會拒絕，但等使用者打完一整份才退回太晚了
  assert.match(
...
test("定稿在留存清單上標示得出來", () => {
  /*
    Info: (20260806 - Julian) 橫幅會說「本期已有定稿報表（見下方留存清單）」；
...
// Info: (20260806 - Julian) ── 軌跡必須留下未截斷的完整內容 ────────────────────────────

test("每一種軌跡動作都保存完整快照，而非只留截斷的摘要", () => {
  /*
    Info: (20260806 - Julian) 軌跡摘要會截斷（列表塞滿整段敘述就沒人會讀），所以截斷過的摘要
  ...
  */
  ...
  /*
    Info: (20260806 - Julian) 「每種動作各自帶對的 detail／snapshot」改由 `report-audit.test.ts` 以
...
test("回填腳本與應用程式共用同一份期間鍵算法", () => {
  /*
    Info: (20260806 - Julian) 先前腳本是手抄的第二份實作，靠註解約定一致。兩份分岔的後果是
...
test("契約外同名項目不得以名稱當唯一鍵", () => {
  /*
    Info: (20260806 - Julian) 以 `x:${itemName}` 當 Map 鍵時，同名兩列會靜默收斂成最後一筆：
...
// Info: (20260806 - Julian) ── 軌跡的摘要與快照分欄，不靠嗅探 ──────────────────────────

test("軌跡不得以換行切分摘要與快照", () => {
  /*
    Info: (20260806 - Julian) 摘要含使用者原文，而原文可以有換行：`施工概況：上午澆置\n下午養護`
...
// Info: (20260806 - Julian) ── 數量表備註的往返 ────────────────────────────────────────

test("數量表備註必須能讀出、顯示並送回", () => {
  /*
    Info: (20260806 - Julian) note 若只寫得進 DB 而表單讀不到，使用者開啟日報存個檔就會把它寫成 null
...
// Info: (20260806 - Julian) ── 期間身分、時鐘與請求競態 ────────────────────────────────

test("期間身分用文字鍵，不用時間相等比對", () => {
  /*
    Info: (20260806 - Julian) periodStart 由伺服器時區推導。部署時區一改（UTC → Asia/Taipei），
  ...
  */
  ...
  for (const fn of [
    ...
    // Info: (20260806 - Julian) 去掉註解再比對：說明文字本來就會提到 periodStart
...
test("基準日只解析一次，不在下游再讀時鐘", () => {
  // Info: (20260806 - Julian) 跨月的午夜前後產製，兩次 new Date() 會讓 label 與 periodKey 落在不同月份
  const service = read("src/service/report.service.ts");
...
test("彙整報表端點不接受 DAILY", () => {
  /*
    Info: (20260806 - Julian) 日報是監造人工填報的 SupervisionReport，與 GeneratedReport 是兩種東西。
...
test("重疊的產製請求只採用最後一次", () => {
  // Info: (20260806 - Julian) 點週報（慢）再點月報（快）→ 週報後到會覆寫畫面與 savedId
  const source = read("src/app/logs/report-generator.tsx");
...
test("每一份草稿都可刪除，包含畫面上顯示的那一份", () => {
  // Info: (20260806 - Julian) 這是唯一的清理路徑；對正在看的那一列不可用等於沒有
  const ui = read("src/app/logs/report-archive.tsx");
  assert.ok(
...
// Info: (20260806 - Julian) ── 不變式由資料庫與交易保證，不只靠服務層 ──────────────────

test("同期只允許一份定稿由資料庫的唯一約束保證", () => {
  /*
    Info: (20260806 - Julian) 服務層的「先查有沒有定稿、再寫」是 check-then-write：
  ...
  */
  ...
  // Info: (20260806 - Julian) 競態下由資料庫擋下，服務層要把它轉成可讀訊息而非 500
  assert.match(read("src/service/report.service.ts"), /isUniqueViolation/);
});

test("日報異動與其稽核軌跡寫在同一個交易", () => {
  /*
    Info: (20260806 - Julian) 分開兩次寫入時，日報寫成功而軌跡沒寫成，那次變動在系統中等於沒發生過
  ...
  */
  ...
  // Info: (20260806 - Julian) 服務層不得再自行分開寫入軌跡
...
test("與 reportDate 比較的期間邊界一律換算成同一基準", () => {
  /*
    Info: (20260806 - Julian) reportDate 由 `new Date("YYYY-MM-DD")` 產生，JS 對純日期字串以 UTC 午夜解析；
  ...
  */
  ...
  // Info: (20260806 - Julian) 參數位置比對，不綁定換行位置 —— 換個排版不該讓守門失效
  assert.match(
    service,
  ...
  /*
    Info: (20260806 - Julian) 整批取數一起檢查，而不是逐個列舉 —— 先前就是「三個用 qEnd、一個用 end」
