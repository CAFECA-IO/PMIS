...
/** Info: (20260806 - Julian) 報表名稱（沿用既有用語，供標題與 prompt）。 */
...
/** Info: (20260806 - Julian) 工期概況；契約工期未填時 total/remaining 為 null，呼叫端應顯示「—」。 */
export interface DurationSummary {
  /** Info: (20260806 - Julian) 契約工期（工作天）；未填為 null */
  total: number | null;
  /** Info: (20260806 - Julian) 累積工期（自開工日至期末的日曆天數）；無開工日為 null */
  elapsed: number | null;
  /** Info: (20260806 - Julian) 剩餘工期 = total − elapsed；任一為 null 時為 null */
  remaining: number | null;
  /** Info: (20260806 - Julian) 使用比例（%，四捨五入至小數 1 位）；無法計算時為 null */
...
/**
 * Info: (20260806 - Julian) 停工原因（決策 H）。
 *
...
/** Info: (20260806 - Julian) 逐日日誌用於工作日判定的最小欄位。 */
export interface DailyLogLike {
  reportDate: Date;
  /** Info: (20260806 - Julian) 僅供紀錄，**不參與**工作日判定（決策 D）。 */
  weather: string | null;
  summary: string | null;
  /** Info: (20260806 - Julian) 停工原因；null 代表當日有施工。決策 H 後為判定的權威來源。 */
  stopReason?: StopReason | null;
  /** Info: (20260806 - Julian) 是否免計工期（E5）；具法律效果，由監造宣告，系統不推測。 */
  excludedFromDuration?: boolean;
}

/** Info: (20260806 - Julian) 單日分類。 */
...
export interface WorkDayStats {
  ...
  unclassified: number;
  /**
   * Info: (20260806 - Julian) 免計工期天數（E5）。
   *
   ...
   */
  excludedDays: number;
  /** Info: (20260806 - Julian) 期間內的日報篇數（非曆日數）。 */
  total: number;
}

// Info: (20260804 - Julian) 判定用關鍵詞抽為常數，避免散落魔法字串；
// Info: (20260806 - Julian) 此規則影響工期展延爭議，實務上需由監造確認後再調整。
...
/**
 * Info: (20260806 - Julian) 單日分類規則（決定論）。
 *
...
export function summarizeWorkDays(logs: DailyLogLike[]): WorkDayStats {
  ...
  for (const log of logs) {
    ...
    // Info: (20260806 - Julian) 免計工期獨立累計，與分類正交（施工日亦可能免計，如部分停工）
    if (log.excludedFromDuration) stats.excludedDays += 1;
  }
  return stats;
}

/** Info: (20260806 - Julian) 進度增量計算所需的履約事項最小欄位。 */
...
/**
 * Info: (20260806 - Julian) 截取 S-Curve 顯示區間：保留「期末所在點」及其之前共 count 個點。
...
/** Info: (20260806 - Julian) 月度分桶標籤（與 buildSCurve 一致的 `YYYY/MM` 格式）。 */
export function monthLabel(d: Date): string {
  return `${d.getFullYear()}/${String(d.getMonth() + 1).padStart(2, "0")}`;
}

/** Info: (20260806 - Julian) 落差描述：正為超前、負為落後、0 為符合預定。回傳如「超前 9.99 個百分點」。 */
