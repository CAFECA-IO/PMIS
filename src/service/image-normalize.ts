/**
 * Info: (20260806 - Julian) HEIC／HEIF → JPEG 影像正規化。
 *
...
/** Info: (20260806 - Julian) 僅供伺服器端使用（動態載入 heic-convert）。 */

/**
 * Info: (20260806 - Julian) 可解碼的 HEIF 品牌（HEVC 編碼的影像／影像序列）。
 *
...
/** Info: (20260806 - Julian) 轉檔輸出品質。0.85 在檔案大小與可辨識度之間取平衡。 */
const JPEG_QUALITY = 0.85;

/**
 * Info: (20260806 - Julian) 允許轉檔的輸入上限（位元組）。
 *
...
/** Info: (20260806 - Julian) 轉檔後的 MIME 與副檔名。 */
export const CONVERTED_MIME = "image/jpeg";
export const CONVERTED_EXT = "jpg";

/** Info: (20260806 - Julian) HEIC／HEIF 的 MIME（供白名單放行；實際判定仍以 magic bytes 為準）。 */
...
/** Info: (20260806 - Julian) HEIC／HEIF 的副檔名（供 accept 屬性與 MIME 缺失時的輔助判定）。 */
...
/**
 * Info: (20260806 - Julian) 以 ISO-BMFF 的 ftyp box 判定是否為 HEIF 家族影像。
 *
...
export function sniffHeif(bytes: Uint8Array): boolean {
  ...
  // Info: (20260806 - Julian) box size 為大端 32 位元；異常值時僅檢查主品牌（上方已做）
  const boxSize =
    ((bytes[0] << 24) | (bytes[1] << 16) | (bytes[2] << 8) | bytes[3]) >>> 0;
  if (boxSize < 16 || boxSize > bytes.byteLength) return false;

  // Info: (20260806 - Julian) 相容品牌自 offset 16 起，每 4 bytes 一個
...
/** Info: (20260806 - Julian) 副檔名是否為 heic／heif（MIME 缺失時的輔助線索，非判定依據）。 */
...
/** Info: (20260806 - Julian) MIME 是否宣告為 HEIF 家族。 */
...
/** Info: (20260806 - Julian) 把 .heic／.heif 副檔名換成 .jpg；無該副檔名者附加。 */
...
export type NormalizeResult =
  | {
      ...
      mimeType: string;
      fileName: string;
      /** Info: (20260806 - Julian) 是否實際做了轉檔（非 HEIC 輸入為 false，原樣通過）。 */
      converted: boolean;
    }
  | {
      ok: false;
      /** Info: (20260806 - Julian) 失敗原因代號，供呼叫端決定訊息；不直接外洩解碼器訊息。 */
      reason: "too_large" | "decode_failed";
    };

/**
 * Info: (20260806 - Julian) 若輸入為 HEIC／HEIF 則轉為 JPEG，否則原樣回傳。
 *
...
export async function normalizeImage(
  ...
  // Info: (20260806 - Julian) 非 HEIF：原樣通過，不載入解碼器
  if (!looksHeif) {
    return { ok: true, bytes, mimeType, fileName, converted: false };
  }

  // Info: (20260806 - Julian) 宣告為 HEIF 但內容不是（副檔名或 MIME 誤標）：交還原始位元組，
  // Info: (20260806 - Julian) 由既有白名單判斷。硬送進解碼器只會得到無意義的失敗。
  if (!sniffHeif(bytes)) {
    return { ok: true, bytes, mimeType, fileName, converted: false };
  }
  ...
  try {
    ...
    // Info: (20260806 - Julian) libheif 會把解析細節印到 console；此處僅回代號，不轉述其訊息
