 ...
 *
 * Info: (20260804 - Luphia) 另提供「AI 任務」機制——各功能模組（專案建置、預警規則…）不再自建對話框，
...
/** Info: (20260804 - Luphia) 串流事件：型別由各任務自行判讀，此處僅約定必有 type。 */
export type AiStreamEvent = { type: string } & Record<string, unknown>;

/**
Info: (20260804 - Luphia) /**
 * 通用「多步驟進度」的單一步驟。
 *
...
export type FaithStep = {
  /** Info: (20260804 - Luphia) 穩定鍵，用於對應同一步驟的 start/done。 */
  key: string;
  /** Info: (20260804 - Luphia) 顯示名稱，如「規劃工程分項」。 */
  label: string;
  status: "active" | "done";
  /** Info: (20260804 - Luphia) 開始時間（ms）；供呈現端計算進行中步驟的即時耗時。 */
  startedAt?: number;
  /** Info: (20260804 - Luphia) 完成耗時（ms）；done 時填入。 */
  elapsedMs?: number;
  /** Info: (20260804 - Luphia) 補充說明，如完成數量。 */
  detail?: string;
};

/**
 * Info: (20260804 - Luphia) 事件的去向。刻意區分「暫時的工作狀態」與「留在對話中的訊息」：
...
/**
 * Info: (20260810 - Luphia) 任務回應：reply 顯示於對話中，其餘欄位由 onResult
 * 交還功能端。
 *
 * 這是信封的 **payload**，不是整個回應 —— 錯誤文案在信封的 `message`，
 * 由 ai-panel 取出後丟出，不會走到 onResult。故此處刻意不含 `error`。
 */
export type AiTaskResponse = { reply?: string } & Record<string, unknown>;

export type AiTask = {
  /** Info: (20260804 - Luphia) 任務識別，用於避免重複啟動 */
  id: string;
  /** Info: (20260804 - Luphia) 顯示於費思標題列，如「專案建置」 */
  title: string;
  /** Info: (20260804 - Luphia) 進入任務時的開場白（Markdown） */
  greeting: string;
  /** Info: (20260804 - Luphia) 任務專用 API 路徑 */
  endpoint: string;
  /** Info: (20260804 - Luphia) 組出送往 endpoint 的 body，可帶入目前草稿作為上下文 */
  /**
   * Info: (20260804 - Luphia) 組出任務 API 的請求主體。回傳物件型別（而非 unknown），
   * 讓面板能再併入通用欄位（如目前鎖定的 projectId）。
   */
  buildBody: (req: AiTaskRequest) => Record<string, unknown>;
  /**
   * Info: (20260804 - Luphia) 收到成功回應時交還功能端，用於填入表單。
   *
  ...
  /**
   * Info: (20260804 - Luphia) 以 NDJSON 串流回應（每行一個事件）。
   * 用於分段解析這類耗時工作，讓費思能邊做邊回報，
   * 而非等到全部結束才一次交付。
   */
  stream?: boolean;
  /**
   * Info: (20260804 - Luphia) 串流模式下每收到一個事件即呼叫，由任務決定該事件的去向。
   * 進度類事件應回 activity（不留存），最終結果回 message（留在對話）。
   */
  onEvent?: (event: AiStreamEvent) => AiEventOutcome;
  /** Info: (20260804 - Luphia) 可上傳的檔案格式（<input accept>），未指定則不顯示上傳 */
  accept?: string;
  /** Info: (20260804 - Luphia) 快速提示詞 */
  suggestions?: string[];
};
...
type Ctx = {
  ...
  /** Info: (20260804 - Luphia) 目前進行中的 AI 任務，null 代表一般問答模式 */
  task: AiTask | null;
  /** Info: (20260804 - Luphia) 啟動任務並自動展開費思 */
  startTask: (task: AiTask) => void;
  /** Info: (20260804 - Luphia) 結束任務，回到一般問答 */
  endTask: () => void;
  /**
   * Info: (20260804 - Luphia) 費思是否正在處理。
   *
  ...
  /**
   * Info: (20260804 - Luphia) 目前畫面上的建置表單提供的協助入口。
   *
   * 讓右下角的費思在建置畫面上「點下去就等於啟動 AI 協助」，
   * 而不是開啟一個與眼前表單無關的一般問答。
   */
  offer: AiAssistOffer | null;
  /** Info: (20260804 - Luphia) 註冊協助入口，回傳解除註冊的函式（供元件卸載時呼叫）。 */
  registerOffer: (offer: AiAssistOffer) => () => void;
};

/** Info: (20260804 - Luphia) 建置表單交給費思的協助入口。 */
export type AiAssistOffer = {
  /** Info: (20260804 - Luphia) 對應的任務 id，用於判斷是否已在進行中。 */
  taskId: string;
  /** Info: (20260804 - Luphia) 表單標題，顯示於右下角。 */
  title: string;
  /** Info: (20260804 - Luphia) 啟動協助。 */
  start: () => void;
};
...
export function AiAssistantProvider({ children }: { children: ReactNode }) {
  ...
  /*
    Info: (20260804 - Luphia) 以 ref 讀取當下狀態，讓 registerOffer 的識別保持穩定 ——
  ...
  */
  ...
  /*
    Info: (20260804 - Luphia) 後註冊者覆蓋先前者：同一時間畫面上通常只有一張建置表單，
    解除時只清掉自己註冊的那一份，避免後開的表單被先關的表單清掉。
  */
  const registerOffer = useCallback((next: AiAssistOffer) => {
    setOffer(next);

    // Info: (20260804 - Luphia) 費思已開啟＝使用者已表態要 AI 參與，此時直接接手（判準見 shouldAutoAssist）
    if (
      shouldAutoAssist({
  ...
  const endTask = useCallback(() => {
    setTask(null);
    // Info: (20260804 - Luphia) 結束任務時一併解除工作中狀態，否則交出表單的一方會永遠停在鎖定
    setWorking(false);
  }, []);
