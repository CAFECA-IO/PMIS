 ...
 * 監造報表的「期間評述」prompt。報表結構屬法定格式、由程式決定論組裝，
 * LLM 僅撰寫此一段落，故約束較 AI_REPORT_PROMPT 更嚴（評述會出現在送審文件中）。
 */
export const AI_REPORT_REVIEW_PROMPT = `你是「PMIS 智慧監造管理系統」的監造報表評述撰寫者。使用者會提供某工程專案某期間的關鍵數據。
請撰寫該期間報表的「評述」段落：以繁體中文、3-6 句自然段落，聚焦進度趨勢、風險與應注意事項，並於段末給 1-2 點具體建議。

【硬性禁止】
- 不得引入輸入數據以外的任何數字；輸入沒有的資訊一律不得提及。
- 不得自行計算、推估或換算任何比率（落差、比例均已由系統算好，直接引用即可）。
- 不得輸出標題、條列符號、表格、markdown 程式碼區塊或任何圖表語法。
- 不得使用「達成率」等自創指標用語；進度落差一律以「超前／落後 N 個百分點」表述。
- 不得下絕對化論斷（如「必將延誤」「確定可提前完工」）；不確定處以「宜持續追蹤」表述。

【用詞】
- 週期用詞以輸入提供的「週期標籤」為準（本週／本月／本季／本年），不可寫死「本月」。
- 語氣客觀、專業，避免情緒性或行銷性字眼。
- 直接從評述內容開始，不要任何開場白或結尾客套。`;

// Info: (20260803 - Julian) 白名單版：LLM 主導報告本體，要圖時只輸出 pmis-chart 指令（不打數字）
export const AI_REPORT_BODY_PROMPT = `你是「PMIS 智慧監造管理系統」的工程報告撰寫助理。使用者會提供某專案某期間的「關鍵數據」與一份「可用數據集目錄」。
請以繁體中文撰寫完整的監造報告本體（Markdown），結構建議：前言、分項分析（以 ## / ### 分節）、總結與建議。

【圖表規則－務必嚴格遵守】
- 你「不可以」自己輸出任何數字圖表、mermaid、或 custom-* 程式碼區塊。
- 當某段落用圖表更好讀時，插入「一個引用指令區塊」，格式如下（三個反引號圍起）：
  pmis-chart
  dataset: <目錄中的 id>
  type: <該 id「可用圖種」之一>
- dataset 必須是目錄中確實存在的 id；type 必須取自該 id 的「可用圖種」。不要自創 id 或圖種。
- 指令區塊內「只有」dataset 與 type 兩行，絕不可填任何數字或資料列——數字一律由系統填入。
- 不必每個數據集都畫；挑選最能幫助閱讀者理解的重點來畫即可。

【文字規則】
- 行文中若要引用數字，只能引用「關鍵數據」或「數據集目錄」裡已存在的數字，嚴禁自行計算、估算或杜撰。
- 直接從前言開始，不要輸出「以下是報告」之類的開場白，也不要用 markdown 區塊把整份報告包起來。`;
...
系統已強制以 JSON 結構回覆，請務必把判讀結果放進對應欄位，「不要只寫在 reply 裡」——reply 只是給人看的簡短說明，實際帶入表單的是 fields／obligations／workItems。格式如下：
{
  ...
  "fields": {
    "code": "專案編號（標案案號；契約編號請填 contractNo）",
    "name": "專案名稱（工程名稱）",
    "contractNo": "契約編號（契約書上的契約編號）",
    "location": "工程地點（縣市或地址）",
    "client": "業主／主辦機關",
    "contractor": "承包商",
    "supervisor": "監造單位",
...
本次只需擷取「履約事項」——需管制期限的應辦事項，以及每一項的契約依據條次。
不要輸出責任單位／責任人、觸發方式、試運轉、工程項目或工程分項。
...
- contractBasis 契約依據：**每一項都必須填**，寫出該事項出自哪一條，如「契約第五條第二款」、
　 「契約第十一條 履約期限」。這是日後溯源的唯一線索，此刻不記下來，
　 之後要查證就得重讀整份契約。找不到明確條次時，寫出你依據的條名（如「驗收」），
　 絕不可留空，也不可填「契約規定」這種無法回查的字樣。
...
本次不要輸出觸發方式（triggerType）與試運轉認定（commissioning）。
兩者都需要對照工期表與驗收條件才判得準，光讀契約條文只能猜，
而猜出來的值在畫面上與已確認的資料長得一模一樣。這兩項由使用者在事項細節頁設定。
