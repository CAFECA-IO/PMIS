...
export type SupervisionReportData = {
  ...
  filedBy?: string | null;
  /** Info: (20260806 - Julian) 停工原因；null 代表當日有施工（決策 H）。 */
...
// Info: (20260806 - Julian) ── 一次交易：日報異動與其稽核軌跡（決策 J-b）──────────────

/*
  Info: (20260806 - Julian) 日報的異動與它的軌跡必須同進同出。
...
/** Info: (20260806 - Julian) 軌跡列（形狀與 supervisionReportAudit.repository 的 CreateAuditData 相同）。 */
...
/**
 * Info: (20260806 - Julian) 新建日報、數量表與軌跡，單一交易。
 *
...
/**
 * Info: (20260806 - Julian) 更新日報、（可選）取代數量表、寫入軌跡，單一交易。
 *
...
/**
 * Info: (20260806 - Julian) 刪除日報並寫入軌跡，單一交易。
 *
...
/** Info: (20260806 - Julian) 依工項加總的一列結果；`total` 為 null 代表分組無可加總的數值。 */
...
/**
 * Info: (20260806 - Julian) 日報數量依工程分項加總。
 *
...
export async function sumDailyQtyByWorkItem(
  ...
  return groups.map((g) => ({
    ...
    // Info: (20260806 - Julian) Decimal → number；null 代表該分組無數值，交由呼叫端視同無資料
    total: g._sum.dailyQty === null ? null : Number(g._sum.dailyQty),
  }));
}

/** Info: (20260806 - Julian) 某日報的數量表明細（依表單排序）。 */
...
/**
 * Info: (20260806 - Julian) 以整批取代某日報的數量表。
 *
...
/**
 * Info: (20260806 - Julian) 跨專案的日報數量加總（供專案列表與儀表板等跨案指標）。
 *
...
/**
 * Info: (20260806 - Julian) 某工程分項已被幾筆日報數量列引用（決策 L）。
 *
