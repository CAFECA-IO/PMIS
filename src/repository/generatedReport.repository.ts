...
export type CreateGeneratedReportData = {
  ...
  /** Info: (20260806 - Julian) 期間身分（如 `MONTHLY:2026-08`）；不受伺服器時區影響。 */
  periodKey: string;
  periodStart: Date;
  periodEnd: Date;
  periodLabel: string;
...
/** Info: (20260806 - Julian) 產出時間戳；覆寫既有草稿時一併更新，故不能靠 createdAt。 */
const stamp = () => new Date();

/**
 * Info: (20260806 - Julian) 建立或覆寫草稿留存 —— 這是 `GeneratedReport` 唯一的寫入入口。
 *
...
export async function upsertDraft(data: CreateGeneratedReportData) {
  /*
    Info: (20260806 - Julian) 期間鍵是留存的身分，空字串會讓不同期間互相覆寫。
  ...
  */
  ...
  /*
    Info: (20260806 - Julian) 取最新的一筆。`findFirst` 不給 orderBy 時的順序是不確定的，
  ...
  */
  ...
  /*
    Info: (20260806 - Julian) ── 為何是 updateMany 而不是 update ────────────────────────
  ...
  */
  if (existing) {
    ...
    if (count > 0) {
      // Info: (20260806 - Julian) updateMany 不回傳資料列，故補查一次；查不到代表剛好被刪，往下新建
...
/** Info: (20260806 - Julian) 清單用的欄位；刻意不含 `markdown`（見 listByProject）。 */
const LIST_SELECT = {
  id: true,
  type: true,
  periodStart: true,
...
/**
 * Info: (20260806 - Julian) 某專案的報表留存，依**產出時間**新到舊。
 *
...
export async function listByProject(projectId: string, draftTake = 30) {
  /*
    Info: (20260806 - Julian) 定稿與草稿分開查再合併，而非單一 take。
...
/** Info: (20260806 - Julian) 同一專案、同期間的草稿留存（同期只保留一份）。 */
export function findDraftForPeriod(projectId: string, periodKey: string) {
  // Info: (20260806 - Julian) 同 upsertDraft：明確取最新一筆，避免同期多份草稿時取到不確定的那個
...
/**
 * Info: (20260806 - Julian) 同一專案、同期間是否已有定稿（同期只允許一份 CONFIRMED）。
 *
...
/** Info: (20260806 - Julian) Prisma 唯一約束違反。 */
