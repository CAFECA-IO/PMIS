...
export async function POST(request: Request) {
  // Info: (20260730 - Luphia) 此路由先前完全未驗證，任何人都能呼叫 AI 並上傳檔案
  ...
  try {
    const body = (await request.json()) as {
      ...
      /** Info: (20260730 - Luphia) 目前鎖定的專案（側邊欄 ?project=），未鎖定時歸為未指派。 */
      projectId?: string | null;
      /** Info: (20260730 - Luphia) 對話與本次送出的識別，供互動紀錄與評價對應。 */
      conversationId?: string;
      turnId?: string;
    };

    // Info: (20260730 - Luphia) 先歸檔再送模型：即使判讀失敗，使用者上傳的檔案仍留存於檔案管理
    const { archived, archiveError } = await archiveAttachment(body.attachment, {
      projectId: body.projectId ?? null,
      prompt: lastUserText(body.messages),
    });

    // Info: (20260730 - Luphia) 於紀錄脈絡內執行，模型閘道即可把這次往返歸屬到同一對話與同一次送出
    const { text, note } = await withLogContext(
      ...
      async () => {
        ...
        // Info: (20260730 - Luphia) 先判斷這個問題需不需要調閱本專案的資料，再據以回答。
        // Info: (20260730 - Luphia) 檢索失敗不會拋錯，只會回「沒讀到東西」，對話照常進行。
    ...
    // Info: (20260730 - Luphia) note 是「參考了哪些資料」的說明，由面板附在回答下方以供追溯
    return jsonOk({ text, note, archived, archiveError });
  } catch (error) {
